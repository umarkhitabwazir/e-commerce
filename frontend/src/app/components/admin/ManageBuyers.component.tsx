...
const ManageBuyersComponent = () => {
    ...
    const handleBlock = async (id: string) => {
        try {
            ...
            setError("Failed to block buyer");
        }
    };
  ...
  try {
            ...
            setError("Failed to approved buyer");
        }
    }
    useEffect(() => {
        fetchBuyers();
        ...
        <tbody>
          {buyers.map((buyer) => (
            <tr key={buyer?._id} className="border-t border-gray-700">
              <td className="p-2">{buyer?.username}</td>
              <td className="p-2 break-words">{buyer?.email}</td>
              <td className="p-2 capitalize">{buyer?.status}</td>
