export const orderConfirmationTemp = (order, userName) => {
  const productList = order.products
    .map(
      (item, index) => `
        <tr>
          <td style="padding:8px;border:1px solid #ddd;">${index + 1}</td>
          <td style="padding:8px;border:1px solid #ddd;">${item.productId?.name || "Product"}</td>
  ...
  <body>
      <div class="container">
          ...
          <p style="font-size: 15px; color: #555; margin-top: 20px;">
              ...
              <strong>Total:</strong> <span style="color:#007bff;">$${order.totalPrice.toFixed(2)}</span>
          </p>

          <a href="https://saadicollection.shop/orders/${order._id}" class="btn">View Order</a>

          <p class="footer">
              © ${new Date().getFullYear()} SAADiCcollection.shop. All rights reserved.
          </p>
      </div>
  </body>
  </html>
